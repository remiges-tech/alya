...
func TestBatchDone(t *testing.T) {
	...
	err = batch.MigrateDatabase(conn.Conn())
	assert.NoError(t, err)
	...
	err = redisClient.Set(context.Background(), batch.GetBatchStatusRedisKey(batchID), string(batchsqlc.StatusEnumSuccess), time.Hour).Err()
	assert.NoError(t, err)
