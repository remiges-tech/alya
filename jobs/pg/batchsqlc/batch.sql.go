...
const resetRowsToQueued = `-- name: ResetRowsToQueued :exec
UPDATE batchrows
SET status = 'queued'
...
func (q *Queries) ResetRowsToQueued(ctx context.Context, dollar_1 []int64) error {
	_, err := q.db.Exec(ctx, resetRowsToQueued, dollar_1)
	return err
}
